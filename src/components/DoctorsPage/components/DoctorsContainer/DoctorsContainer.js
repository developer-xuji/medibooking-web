  ...
  render() {
    ...
    if (this.props.IfMore) {
      moreButton = (
        <MoreButton
          onMoreButtonClick={() => {
            this.props.handleMoreClick();
          }}
        />
      );
    }
    const renderdoctor = this.props.CurrentDoctorsList.slice(
      0,
      this.props.CurrentNumOfDoctorsShowed
    ).map((value, index) => (
      <DoctorOfEach
        DoctorId={value}
        Doctor={this.props.AllDoctors[value]}
      ></DoctorOfEach>
    ));
    return (
