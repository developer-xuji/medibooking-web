            ...
            response = response.filter(
                (doctor) =>
                    ...
                    doctor.specializations.filter(
                        (specialization) =>
                            specialization.id === this.props.specializationID).length > 0)
            this.setState({
                doctorList: response
            })
        })
                ...
                <><h1>Similar Doctors</h1>
                    <CardContainer>
                        {doctorList.map((doctor) =>
                            <DoctorCard key={doctor.id} id={doctor.id} firstName={doctor.firstName} lastName={doctor.lastName} gender={doctor.gender} />)}
                    </CardContainer>  </>
            }
        </Layout>
    }
